      ...
      <svg className=" w-full" width={width} height={height} viewBox={viewBoxValue}>
        <path
          ...
          style={{
            ...
            strokeWidth: '3px',
            strokeDasharray: drawValue.set + drawValue.offset + ' ' + pathLength,
...
const caculateDashArray = (progressObject: progressObject[], throPercent: number) => {
  let linePercent = 0
  ...
  for (let i = 0; i < progressObject.length; i++) {
    if (throPercent < progressObject[i].percent) {
      ...
      linePercent =
        ...
        (progressObject[i].line - progressObject[i - 1].line) * percentCalculator
      progressPercent =
        progressObject[i - 1].flow +
        (progressObject[i].flow - progressObject[i - 1].flow) * percentCalculator
      break
    }
...
const percent = (number: number, per: any) => {
